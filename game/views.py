...
class ResultView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        game_obj = GameSession.objects.filter(user=self.request.user).order_by('-time')[:1][0]
        high_scores = GameSession.objects.all().order_by('-score')[:3]
        lower_results = GameSession.objects.filter(score__lte=game_obj.score).exclude(pk=game_obj.pk).order_by('-score', '-time')[:1]
        higher_results = GameSession.objects.filter(score__gte=game_obj.score).exclude(pk__in=[game_obj.pk, lower_results[0].pk]).order_by('score', 'time')[:1]
